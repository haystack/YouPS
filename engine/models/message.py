...
from http_handler.settings import BASE_URL, NYLAS_ID, NYLAS_SECRET, time_entity_extractor
...
class Message(object):
    ...
    def thread(self):
        ...
        for m in nylas.messages.where(thread_id=self._get_nylas_message().thread_id,view='expanded'):
            if BASE_URL not in m.from_[0]["email"]:
    ...
    def is_replied(self):
        ...
        return BaseMessage.objects.filter(imap_account=self._imap_account, _in_reply_to__contains=self._message_id).exclude(from_m__email=self._imap_account.email).exclude(from_m__email__icontains=BASE_URL).exists()
